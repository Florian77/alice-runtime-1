...
} = require("./dashboard-querys");
const {
    getDataContextAggregateList,
    getCommandContextAggregateList,
    getTriggerContextAggregateList,
} = require("./upsert-stats");
const {getDataIndex} = require("./get-data-index");
const {getLastDataEvent} = require("./get-last-data-event");
const {getDataEventStream} = require("./get-data-event-stream");
const {getTrigger} = require("./get-trigger");
const {getCommand} = require("./get-command");
const {setManyCommandsNotPaused} = require("./update-command-state");
const {setManyCommandsPaused} = require("./update-command-state");
const {setOneCommandNotRunning} = require("./update-command-state");
const {setOneCommandNotHandled} = require("./update-command-state");
const {setOneCommandHandled} = require("./update-command-state");
const {setOneCommandNotPaused} = require("./update-command-state");
const {setOneCommandPaused} = require("./update-command-state");
const {storeError} = require("./database");
const {setManyCommandsNotHandled} = require("./update-command-state");
const {setManyCommandsHandled} = require("./update-command-state");
const {
    getProcessState,
    setProcessStateOff,
    setProcessStateOn,
...
async function dashboardApi(action, param) {
    ...
    try {
        ...
        // Data Index
            ...
            //dc.j(query, "query");
            responseBody = {
                resultList: await dashboard_queryDataIndex(query, {page, rowsPerPage,}),
                resultCount: await dashboard_queryDataIndex(query, {getCount: true,}),
        ...
        // Trigger Index
            ...
            responseBody = {
                resultList: await dashboard_queryTriggerIndex(query, {page, rowsPerPage,}),
                resultCount: await dashboard_queryTriggerIndex(query, {getCount: true,}),
        ...
        // ---------------------------------------------------------------------------------
            ...
            responseBody = {
                resultList: await dashboard_queryCommandIndex(query, {page, rowsPerPage,}),
                resultCount: await dashboard_queryCommandIndex(query, {getCount: true,}),
        ...
        // ITEM: Data Index
            ...
            if (R.isNil(context) || R.isNil(context) || R.isNil(aggregateId)) {
                ...
                responseBody = {
                    itemDataIndex: await getDataIndex({context, aggregate, aggregateId,}),
                };
            }
        }
        ...
        // ITEM: Trigger Index
            ...
            if (R.isNil(triggerId) || R.isEmpty(triggerId)) {
                ...
                responseBody = {
                    itemTriggerIndex: await getTrigger({triggerId})
                };
            }
        }
        ...
        // ITEM: Command Index
            ...
            if (R.isNil(commandId) || R.isEmpty(commandId)) {
                ...
                responseBody = {
                    itemCommandIndex: await getCommand({commandId})
                };
            }
        }
