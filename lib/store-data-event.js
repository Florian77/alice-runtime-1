...
async function storeDataEvent({
    ...
    const contextSequenceNumber = await nextSequenceNumber_dataStream(contextStreamId);

    const systemStreamId = "__SYSTEM";
    const systemSequenceNumber = await nextSequenceNumber_dataStream(systemStreamId);
    ...
    if (!dataIndexUpdated) {
        const dataIndexSequenceNumber = await nextSequenceNumber_dataIndex();
