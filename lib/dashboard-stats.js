...
const {getCommandControl} = require("./command-control");


async function getUndispatchedDataEventCount() {
    const filter = {
        dispatched: false,
    };
...
async function getTriggerCheckForUpdatesCount() {
    const filter = {
...
async function getUnhandledCommandCount() {
    const filter = {
        handled: false,
        running: false
    };
...
function contextAggregateQuery(collection, match, countName) {
    return async ({context, aggregate, command}) => {
...
async function contextAggregateListQuery(collection, match, countName, contextAggregateList) {
...
function makeIndexKey(d) {
    return R.join("/", [d.context, d.aggregate, d.command]);
}


function makeIndexObject(data, defaultObject = {}) {
    return R.pipe(
        R.map(
...
function addIndexObjectData(indexObject, newData) {
    newData.forEach(d => {
...
async function getCommandIndexOverview(options = {commandList: false}) {
...
async function getTriggerIndexOverview() {
...
async function getDataIndexOverview() {
