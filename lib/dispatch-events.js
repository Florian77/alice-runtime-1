...
async function findEventToDispatch({
                                       context = null,
    ...
    // console.log("findEventToDispatch:", R.pathOr(null, ["value", "_id"], result))
    return result;
}


async function updateTriggerIndex(event) {
    const filter = {
        type: "stream",
        streamType: "DATA",
        ...
        $or: [
            {
                "streamId": makeStreamId(R.pick(["context", "aggregate", "aggregateId"], event)),
            },
            {
                "streamId": makeStreamId(R.pick(["context", "aggregate"], event)),
            },
            {
                "streamId": makeStreamId(R.pick(["context"], event)),
            },
        ]
    };
    const update = {
